...
namespace BattleSystem.Fighter
{
    public static class BattleSpawner
    {
            ...
            bool isLeader = false)
        {
            ...
            if (fighterId > 0)
            {
                var fighterConfig = TribeSystem.TribeConfigLoader.Instance?.GetFighterConfig(fighterId);
            ...
            {
                var buffConfig = TribeSystem.TribeConfigLoader.Instance?.GetBuffConfig(buffId);
