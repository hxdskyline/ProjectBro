...
/// <summary>
/// 独立的游历面板（仅负责 UI 根、列表容器与确认/关闭按钮），
/// 由上层（如 CardBuildPanel）负责往 `ListRoot` 填充卡片项并处理具体逻辑。
/// </summary>
public class OutingPanel
{
    ...
    public void Initialize(RectTransform parent, Font uiFont)
    {
        ...
        Root = GetOrCreateChildRect(parent, "OutingPanelRoot", new Vector2(0.53f, 0.08f), new Vector2(0.96f, 0.72f));
        Image popupBg = GetOrAddComponent<Image>(Root.gameObject);
        popupBg.color = new Color(0.09f, 0.14f, 0.2f, 0.95f);
        ...
        HintText.text = "选择两只猫进行游历；游历返回的新卡会加入待选区。";
        ...
        ConfigureOutingLayout(ListRoot);

        ConfirmButton = GetOrCreateButton(Root, "ConfirmButton", "确认游历", _uiFont, new Vector2(0.52f, 0.86f), new Vector2(0.74f, 0.96f), new Color(0.55f, 0.18f, 0.18f, 1f));
    ...
    private void ConfigureOutingLayout(RectTransform zoneRoot)
    {
        if (zoneRoot == null) return;
        VerticalLayoutGroup oldVert = zoneRoot.GetComponent<VerticalLayoutGroup>();
        if (oldVert != null) Object.Destroy(oldVert);
        ...
        horiz.padding = new RectOffset(12, 12, 12, 12);
    }
